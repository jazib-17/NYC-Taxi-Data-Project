...
lasso_pred <- predict(lasso_cv, s = lasso_cv$lambda.1se, newx = X_test)

lasso_train_pred <- predict(lasso_cv, s = lasso_cv$lambda.1se, newx = X_train)
cat("RMSE lambda.1se :", sqrt(mean((Y_train - lasso_train_pred)^2)), "\n")

#write to submission file
outDat = data.frame(uid = test_dat$uid, fare_amount = as.vector(lasso_pred))

write.csv(outDat, "W26P1_submission1.csv", row.names = F)
